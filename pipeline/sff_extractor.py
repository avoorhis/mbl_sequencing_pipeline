    ...
    for sff_file in sff_files:
        ...
        for seq_data in sequences(fileh=sff_fh, header=header_data):
            ...
            if ssahapematches.has_key(seq_data['name']):
                ...
                whole_shebang[seq_data['name']] = {'sequence' : seq_data['bases'], 'quality' : seq_data['quality_scores'] }
                nseqs_out += 1
        #print "done."
...
def run_sff_extractor_for_pipeline(sff_extract_config,sff_files):

    global config
    config = sff_extract_config
    basename = 'reads'
    def_seq_fname = basename + '.fasta'
    def_qual_fname = basename + '.fasta.qual'
    def_xml_fname = basename + '.xml'
    ...
    reads = extract_reads_from_sff(config,sff_files)
    return reads
